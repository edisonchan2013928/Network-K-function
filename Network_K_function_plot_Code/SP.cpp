...
void init_Dijkstra(model& our_model, PQ& pq)
{
	...
	if (our_model.method == 1) //RQS method
	{
		...
		vector<Point>& PS = our_model.edge_set[cur_edge_index].PS_vec[our_model.cur_data_index];
	...
	if (our_model.method >= 2 && our_model.method <= 6) //SPS, CA, NS, CA^{(ASPS)}, and NS^{(ASPS)} methods
	{
...
void Dijkstra(model& our_model, vector<sp_node>& sp_node_vec, vector<int>& access_edge_list, bool*is_scan_edge_list)
{
	...
	while (pq.size() > 0)
	{
		...
		if (sp_node_vec[SP_node.node_index].cur_sp_value > our_model.tau_vec[our_model.cur_tau_index])
			continue;
