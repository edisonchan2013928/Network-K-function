...
double point_edge_count_same_edge(model& our_model, int p_index, int e_index)
{
	...
	if (our_model.method == 1 || our_model.method == 2)
	{
		for (int p = 0; p < (int)PS.size(); p++)
		{
			...
			if (delta <= our_model.tau_vec[our_model.cur_tau_index])
				edge_K_value++;
		}
	}
	...
	if (our_model.method == 3 || our_model.method == 5)
	{
		...
		double dist_left = PS[p_index].dist_n1 - our_model.tau_vec[our_model.cur_tau_index];
		double dist_right = PS[p_index].dist_n2 - our_model.tau_vec[our_model.cur_tau_index];
...
double point_edge_count(model& our_model, int e_index, int node_u, int node_v)
{
	...
	if (our_model.method == 3 || our_model.method == 5) //CA or CA^{(ASPS)}
	{
		vector<Point>& PS_wide_e = our_model.edge_set[our_model.cur_edge_index].PS_vec[our_model.cur_data_index];
		int p_wide_e = our_model.cur_point_index;
		...
		if (dist_p_i_u > our_model.tau_vec[our_model.cur_tau_index] && dist_p_i_v > our_model.tau_vec[our_model.cur_tau_index]) //Case 1
			return 0;
		if (dist_p_i_u <= our_model.tau_vec[our_model.cur_tau_index] && dist_p_i_v > our_model.tau_vec[our_model.cur_tau_index]) //Case 2
		{
			left_pos = edge_binary_search_from_n1(PS, our_model.tau_vec[our_model.cur_tau_index] - dist_p_i_u, 0, PS.size() - 1);
			if (left_pos == -1)
				return 0;
			else
				return left_pos + 1;
		}
		if (dist_p_i_u > our_model.tau_vec[our_model.cur_tau_index] && dist_p_i_v <= our_model.tau_vec[our_model.cur_tau_index]) //Case 3
		{
			right_pos = edge_binary_search_from_n2(PS, our_model.tau_vec[our_model.cur_tau_index] - dist_p_i_v, 0, PS.size() - 1);
			if (right_pos == -1)
				return 0;
			else
				return PS.size() - right_pos;
		}
		if (dist_p_i_u <= our_model.tau_vec[our_model.cur_tau_index] && dist_p_i_v <= our_model.tau_vec[our_model.cur_tau_index]) //Case 4
		{
			left_pos = edge_binary_search_from_n1(PS, our_model.tau_vec[our_model.cur_tau_index] - dist_p_i_u, 0, PS.size() - 1);
			right_pos = edge_binary_search_from_n2(PS, our_model.tau_vec[our_model.cur_tau_index] - dist_p_i_v, 0, PS.size() - 1);
...
double edge_edge_count(model& our_model, int wide_e_index, int e_index)
{
	...
	bool au_end = false;
	bool bu_end = false;
	cur_tau_au = our_model.tau_vec[our_model.cur_tau_index] - our_model.sp_node_vec_first[node_u].cur_sp_value;
	cur_tau_bu = our_model.tau_vec[our_model.cur_tau_index] - our_model.sp_node_vec_second[node_u].cur_sp_value - our_model.edge_set[wide_e_index].length;
	pos_au = -1; pos_bu = -1;
	...
	bool av_end = false;
	bool bv_end = false;
	cur_tau_av = our_model.tau_vec[our_model.cur_tau_index] - our_model.sp_node_vec_first[node_v].cur_sp_value;
	cur_tau_bv = our_model.tau_vec[our_model.cur_tau_index] - our_model.sp_node_vec_second[node_v].cur_sp_value - our_model.edge_set[wide_e_index].length;
	pos_av = -1; pos_bv = -1;
...
double edge_edge_count_same_edge(model& our_model, int e_index)
{
	...
	for (int i = 0; i < (int)PS.size(); i++)
	{
		if (i == 0) //initialization
		{
			...
			for (int ii = 0; ii < (int)PS.size(); ii++)
			{
				if (PS[ii].dist_n1 - PS[i].dist_n1 <= our_model.tau_vec[our_model.cur_tau_index])
					right++;
				else
					break;
			}
		...
		{
			if (PS[i].dist_n1 - PS[ii].dist_n1 <= our_model.tau_vec[our_model.cur_tau_index])
				break;
			else
				left++;
		}
